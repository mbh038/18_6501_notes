...
The next two tests can be applied for generally to data that is not drawn from a gaussian distribution (but some restrictions still apply).
...
Is like Wald's rest in one dimension, but instead of measring whether $\theta$ is close to $\theta_0$ on the $x$ axis, we are measuring whether the likelihood of $\theta$ is close to that of $\theta_0$ on the $y$ axis.
