...
\item $\mathbb{E}\left[V{\sim}\chi^2_d\right]=\mathbb{E}[Z_1^2]+\dots+\mathbb{E}[Z_d^2]=d$
\item $\textsf{var}\left[V{\sim}\chi^2_d\right]=\textsf{var}[Z_1^2]+\dots+\textsf{var}[Z_d^2]=2d$
\end{itemize}
...
\item An unbiased estimator $\tilde S_n$  is
...
$$T_n \sim t_{n-1}.$$
So that the students $T$ test with non-asymptotic level $\alpha\in(0,1)$ is
$$\psi(\alpha)=\mathds{1} \left\{\vert T_n\vert>q_{\alpha/2}\right\}$$
where $q_{\alpha /2}$ is the $(1-\alpha /2)$ quantile of $t_{n-1}$.

\subsubsection{Pros and cons of $T$ test}
Advantage: Non-asymptotic. i.e. can be run on small samples. Will also work on large samples\\
...
Is like Wald's test in one dimension, but instead of measuring whether $\theta$ is close to $\theta_0$ on the $x$ axis, we are measuring whether the likelihood of $\theta$ is close to that of $\theta_0$ on the $y$ axis.
